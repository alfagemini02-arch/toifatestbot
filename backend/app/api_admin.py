...
def _serialize_telegram_group(group: TelegramGroup) -> dict:
    return {
        "id": group.id,
        "chat_id": group.chat_id,
        "title": group.title,
        "is_allowed": group.is_allowed,
...
def list_telegram_groups(db: Session = Depends(get_db)) -> list[dict]:
    groups = list(db.scalars(select(TelegramGroup).order_by(TelegramGroup.created_at.desc())))
    return [_serialize_telegram_group(group) for group in groups]


@router.post("/telegram-groups", status_code=201)
def create_telegram_group(payload: TelegramGroupInput, db: Session = Depends(get_db)) -> dict:
    group = TelegramGroup(
    ...
    try:
        ...
        db.rollback()
        raise HTTPException(status_code=409, detail="Bu guruh ID avval qo'shilgan") from exc
    db.refresh(group)
    return _serialize_telegram_group(group)


@router.put("/telegram-groups/{group_id}")
def update_telegram_group(group_id: int, payload: TelegramGroupInput, db: Session = Depends(get_db)) -> dict:
    group = db.scalar(select(TelegramGroup).where(TelegramGroup.id == group_id))
    if not group:
        raise HTTPException(status_code=404, detail="Guruh topilmadi")
    group.chat_id = payload.chat_id
    group.title = payload.title.strip() if payload.title else None
    group.is_allowed = payload.is_allowed
    try:
        db.commit()
    except IntegrityError as exc:
        db.rollback()
        raise HTTPException(status_code=409, detail="Bu guruh ID boshqa yozuvda mavjud") from exc
    return _serialize_telegram_group(group)


@router.delete("/telegram-groups/{group_id}")
def delete_telegram_group(group_id: int, db: Session = Depends(get_db)) -> dict:
    group = db.scalar(select(TelegramGroup).where(TelegramGroup.id == group_id))
    if not group:
        raise HTTPException(status_code=404, detail="Guruh topilmadi")
    db.delete(group)
