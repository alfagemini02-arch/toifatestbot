...
class User(Base):
    ...
    registered_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), default=utcnow, index=True, nullable=False)
...
class Test(Base):
    ...
    group_quiz_sessions: Mapped[list[GroupQuizSession]] = relationship(back_populates="test")
...
class TelegramGroup(Base):
    ...
    quiz_sessions: Mapped[list[GroupQuizSession]] = relationship(back_populates="group")
...
class GroupQuizSession(Base):
    ...
    id: Mapped[int] = mapped_column(primary_key=True)
    group_id: Mapped[int | None] = mapped_column(ForeignKey("telegram_groups.id", ondelete="SET NULL"), index=True)
    ...
    group: Mapped[TelegramGroup | None] = relationship(back_populates="quiz_sessions")
    test: Mapped[Test | None] = relationship(back_populates="group_quiz_sessions")
    questions: Mapped[list[GroupQuizQuestion]] = relationship(
...
class GroupQuizQuestion(Base):
    __tablename__ = "group_quiz_questions"
    ...
    poll_id: Mapped[str | None] = mapped_column(String(128), unique=True, index=True)
    message_id: Mapped[int | None] = mapped_column(BigInteger)
    sent_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True))

    session: Mapped[GroupQuizSession] = relationship(back_populates="questions")
    answers: Mapped[list[GroupQuizAnswer]] = relationship(back_populates="quiz_question", cascade="all, delete-orphan")

    __table_args__ = (UniqueConstraint("session_id", "order_index", name="uq_group_quiz_order"),)
...
class GroupQuizAnswer(Base):
    ...
    id: Mapped[int] = mapped_column(primary_key=True)
    session_id: Mapped[int] = mapped_column(ForeignKey("group_quiz_sessions.id", ondelete="CASCADE"), index=True, nullable=False)
    quiz_question_id: Mapped[int] = mapped_column(ForeignKey("group_quiz_questions.id", ondelete="CASCADE"), index=True, nullable=False)
    ...
    session: Mapped[GroupQuizSession] = relationship(back_populates="answers")
