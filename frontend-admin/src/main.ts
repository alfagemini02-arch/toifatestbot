...
async function testModal(test?: TestItem): Promise<void> {
  ...
  document.querySelector<HTMLFormElement>('#test-form')!.addEventListener('submit', async e => { e.preventDefault(); const form = new FormData(e.currentTarget as HTMLFormElement); const payload = { name: String(form.get('name')), test_mode: String(form.get('test_mode')), time_limit_minutes: Number(form.get('time_limit_minutes')), is_active: true, rules: [...editor.querySelectorAll<HTMLElement>('.rule-row')].map(row => ({ source_id: Number(row.querySelector('select')!.value), question_count: Number(row.querySelector('input')!.value) })) }; try { await api(`/api/admin/tests${test ? `/${test.id}` : ''}`, { method: test ? 'PUT' : 'POST', body: JSON.stringify(payload) }); closeModal(); toast('✅ Test saqlandi'); showTests(); } catch (error) { document.querySelector('#test-error')!.textContent = error instanceof Error ? error.message : String(error); } });
}
