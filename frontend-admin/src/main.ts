...
async function showTests(): Promise<void> {
  ...
  try { const tests = await api<TestItem[]>('/api/admin/tests'); if (!sourceCache.length) await loadSources(); document.querySelector('#content')!.innerHTML = `<section class="panel"><div class="panel-title"><div><h2>Testlar</h2><span>${tests.length} ta</span></div><button class="primary compact" id="new-test">+ Yangi test</button></div><div class="table-wrap"><table><thead><tr><th>Nomi</th><th>Savollar</th><th>Vaqt</th><th>Ishlangan</th><th>Amallar</th></tr></thead><tbody>${tests.map(t => `<tr><td><strong>${esc(t.name)}</strong><small class="block">${t.rules.map(r => `${esc(r.source_name)}: ${r.question_count}`).join(' · ')}</small></td><td>${t.total_questions}</td><td>${t.time_limit_minutes ? `${t.time_limit_minutes} daq.` : 'Cheksiz'}</td><td>${t.attempt_count}</td><td class="actions"><button data-edit-test="${t.id}">✏️</button><button class="danger-icon" data-delete-test="${t.id}">🗑</button></td></tr>`).join('') || '<tr><td colspan="5" class="empty-cell">Test mavjud emas</td></tr>'}</tbody></table></div></section>`;
...
async function testModal(test?: TestItem): Promise<void> {
  ...
  document.querySelector<HTMLFormElement>('#test-form')!.addEventListener('submit', async e => { e.preventDefault(); const form = new FormData(e.currentTarget as HTMLFormElement); const payload = { name: String(form.get('name')), time_limit_minutes: Number(form.get('time_limit_minutes')), is_active: true, rules: [...editor.querySelectorAll<HTMLElement>('.rule-row')].map(row => ({ source_id: Number(row.querySelector('select')!.value), question_count: Number(row.querySelector('input')!.value) })) }; try { await api(`/api/admin/tests${test ? `/${test.id}` : ''}`, { method: test ? 'PUT' : 'POST', body: JSON.stringify(payload) }); closeModal(); toast('✅ Test saqlandi'); showTests(); } catch (error) { document.querySelector('#test-error')!.textContent = error instanceof Error ? error.message : String(error); } });
}
