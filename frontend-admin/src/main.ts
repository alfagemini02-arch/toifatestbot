...
type TestItem = { id: number; name: string; test_mode: 'exam' | 'classifier'; time_limit_minutes: number; group_question_seconds: number; group_start_vote_count: number; group_start_vote_seconds: number; group_stop_vote_count: number; group_stop_vote_seconds: number; is_active: boolean; total_questions: number; attempt_count: number; rules: TestRule[] };
...
async function showSource(sourceId: number, page = 1, search = '', pageSize = 20, topic = '', difficulty = '', answerCount = ''): Promise<void> {
  ...
  try {
    ...
    const rerun = () => showSource(sourceId, 1, document.querySelector<HTMLInputElement>('#source-search')!.value, Number((document.querySelector<HTMLSelectElement>('#page-size')!.value)), document.querySelector<HTMLSelectElement>('#topic-filter')!.value, document.querySelector<HTMLSelectElement>('#difficulty-filter')!.value, document.querySelector<HTMLSelectElement>('#answer-count-filter')!.value);
    document.querySelector<HTMLInputElement>('#source-search')!.addEventListener('input', () => { clearTimeout(debounceTimer); debounceTimer = window.setTimeout(rerun, 300); });
    ['#topic-filter', '#difficulty-filter', '#answer-count-filter', '#page-size'].forEach(selector => document.querySelector(selector)?.addEventListener('change', rerun));
    document.querySelector('#prev-page')?.addEventListener('click', () => showSource(sourceId, page - 1, search, pageSize, topic, difficulty, answerCount));
    document.querySelector('#next-page')?.addEventListener('click', () => showSource(sourceId, page + 1, search, pageSize, topic, difficulty, answerCount));
  } catch (error) { showError(error); }
}
function questionCard(q: Question, selectable = false): string {
  ...
  const meta = `<div class="question-meta"><span>${esc(q.topic || 'Umumiy')}</span><span>${q.difficulty === 'hard' ? 'Qiyin' : q.difficulty === 'easy' ? 'Oson' : "O'rtacha"}</span><span>${q.answers.length} variant</span></div>`;
  ...
  return `<article class="question-item"><div class="question-head">${selector}<div class="actions"><button data-edit-question="${q.id}">✏️</button><button class="danger-icon" data-delete-question="${q.id}">🗑</button></div></div>${meta}<h3>${esc(q.question_text)}</h3><p class="correct-answer">✅ ${esc(correct?.text || 'To‘g‘ri javob belgilanmagan')}</p>${q.explanation ? `<p class="explanation">Izoh: ${esc(q.explanation)}</p>` : ''}<details><summary>Barcha variantlar (${q.answers.length})</summary>${q.answers.map(a => `<div class="answer-row ${a.correct ? 'correct' : ''}">${a.correct ? '✓' : '•'} ${esc(a.text)}</div>`).join('')}</details></article>`;
}
...
async function questionModal(question?: Question, defaultSourceId?: number, refresh?: () => void | Promise<void>): Promise<void> {
  ...
  openModal(`<form id="question-form" class="wide-modal"><h2>${question ? 'Savolni tahrirlash' : 'Yangi savol'}</h2><label>Manba<select name="source_id">${sourceCache.map(s => `<option value="${s.id}" ${s.id === (question?.source_id || defaultSourceId) ? 'selected' : ''}>${esc(s.name)} (${s.question_count})</option>`).join('')}</select></label><div class="form-grid"><label>Mavzu<input name="topic" value="${esc(question?.topic || '')}" placeholder="Masalan: Bojxona nazorati"></label><label>Qiyinlik<select name="difficulty"><option value="easy" ${question?.difficulty === 'easy' ? 'selected' : ''}>Oson</option><option value="medium" ${!question?.difficulty || question?.difficulty === 'medium' ? 'selected' : ''}>O'rtacha</option><option value="hard" ${question?.difficulty === 'hard' ? 'selected' : ''}>Qiyin</option></select></label></div><label>Savol matni<textarea name="question_text" rows="4" required>${esc(question?.question_text || '')}</textarea></label><label>Tushuntirish<textarea name="explanation" rows="3" placeholder="Javobdan keyin ko'rsatiladigan qisqa izoh">${esc(question?.explanation || '')}</textarea></label><div class="field-label">Javob variantlari</div><div id="answer-editor">${answers.map((a, i) => answerEditorRow(a, i)).join('')}</div><button type="button" class="secondary compact" id="add-answer">+ Variant qo‘shish</button><div class="form-error" id="question-error"></div><div class="modal-actions"><button type="button" class="ghost" data-close>Bekor qilish</button><button class="primary" type="submit">💾 Saqlash</button></div></form>`);
...
async function showTests(): Promise<void> {
  ...
  try { const tests = await api<TestItem[]>('/api/admin/tests'); if (!sourceCache.length) await loadSources(); document.querySelector('#content')!.innerHTML = `<section class="panel"><div class="panel-title"><div><h2>Testlar</h2><span>${tests.length} ta</span></div><button class="primary compact" id="new-test">+ Yangi test</button></div><div class="table-wrap"><table><thead><tr><th>Nomi</th><th>Turi</th><th>Savollar</th><th>Vaqt</th><th>Ishlangan</th><th>Amallar</th></tr></thead><tbody>${tests.map(t => `<tr><td><strong>${esc(t.name)}</strong><small class="block">${t.rules.map(r => `${esc(r.source_name)}: ${r.question_count}`).join(' · ')}</small></td><td><span class="status ${t.test_mode === 'classifier' ? 'warning' : 'active'}">${t.test_mode === 'classifier' ? 'Ajratish sinovi' : 'Oddiy test'}</span></td><td>${t.total_questions}</td><td>${t.time_limit_minutes ? `${t.time_limit_minutes} daq.` : 'Cheksiz'}</td><td>${t.attempt_count}</td><td class="actions"><button data-edit-test="${t.id}">✏️</button><button class="danger-icon" data-delete-test="${t.id}">🗑</button></td></tr>`).join('') || '<tr><td colspan="6" class="empty-cell">Test mavjud emas</td></tr>'}</tbody></table></div></section>`;
    document.querySelector('#new-test')?.addEventListener('click', () => testModal()); document.querySelectorAll<HTMLElement>('[data-edit-test]').forEach(b => b.addEventListener('click', () => testModal(tests.find(t => t.id === Number(b.dataset.editTest))))); document.querySelectorAll<HTMLElement>('[data-delete-test]').forEach(b => b.addEventListener('click', async () => { if (!confirm('Test butunlay o‘chirilsinmi?')) return; try { await api<any>(`/api/admin/tests/${b.dataset.deleteTest}`, { method: 'DELETE' }); toast('Test o‘chirildi'); showTests(); } catch (e) { toast(e instanceof Error ? e.message : String(e), 'error'); } }));
  } catch (error) { showError(error); }
}
async function testModal(test?: TestItem): Promise<void> {
  ...
  openModal(`<form id="test-form" class="wide-modal"><h2>${test ? 'Testni tahrirlash' : 'Yangi test'}</h2><div class="form-grid"><label>Test nomi<input name="name" value="${esc(test?.name || '')}" required></label><label>Test turi<select name="test_mode"><option value="exam" ${test?.test_mode !== 'classifier' ? 'selected' : ''}>Oddiy test</option><option value="classifier" ${test?.test_mode === 'classifier' ? 'selected' : ''}>Savollarni ajratish sinovi</option></select></label><label>Mini App vaqti (daqiqa)<input name="time_limit_minutes" type="number" min="0" max="1440" value="${test?.time_limit_minutes || 0}"></label></div><div class="field-label">Telegram guruh quiz sozlamalari</div><div class="form-grid group-quiz-settings"><label>Har bir savol vaqti (soniya)<input name="group_question_seconds" type="number" min="5" max="600" value="${test?.group_question_seconds ?? 30}" required></label><label>Boshlash uchun ovozlar<input name="group_start_vote_count" type="number" min="1" max="1000" value="${test?.group_start_vote_count ?? 10}" required></label><label>Boshlash ovozi vaqti (soniya)<input name="group_start_vote_seconds" type="number" min="15" max="600" value="${test?.group_start_vote_seconds ?? 120}" required></label><label>To‘xtatish uchun ovozlar<input name="group_stop_vote_count" type="number" min="1" max="1000" value="${test?.group_stop_vote_count ?? 10}" required></label><label>To‘xtatish ovozi vaqti (soniya)<input name="group_stop_vote_seconds" type="number" min="15" max="600" value="${test?.group_stop_vote_seconds ?? 60}" required></label></div><p class="modal-hint">Guruh admini ovoz kutmasdan testni darhol boshlashi yoki to‘xtatishi mumkin.</p><div class="field-label">Manbalar va savollar soni</div><div id="rule-editor">${rules.map((r, i) => testRuleRow(r, i)).join('')}</div><button type="button" class="secondary compact" id="add-rule">+ Manba qo‘shish</button><div class="total-line">Jami: <strong id="test-total">0</strong> ta savol</div><div class="form-error" id="test-error"></div><div class="modal-actions"><button type="button" class="ghost" data-close>Bekor qilish</button><button class="primary" type="submit">💾 Saqlash</button></div></form>`);
  ...
  document.querySelector<HTMLFormElement>('#test-form')!.addEventListener('submit', async e => { e.preventDefault(); const form = new FormData(e.currentTarget as HTMLFormElement); const payload = { name: String(form.get('name')), test_mode: String(form.get('test_mode')), time_limit_minutes: Number(form.get('time_limit_minutes')), group_question_seconds: Number(form.get('group_question_seconds')), group_start_vote_count: Number(form.get('group_start_vote_count')), group_start_vote_seconds: Number(form.get('group_start_vote_seconds')), group_stop_vote_count: Number(form.get('group_stop_vote_count')), group_stop_vote_seconds: Number(form.get('group_stop_vote_seconds')), is_active: true, rules: [...editor.querySelectorAll<HTMLElement>('.rule-row')].map(row => ({ source_id: Number(row.querySelector('select')!.value), question_count: Number(row.querySelector('input')!.value) })) }; try { await api(`/api/admin/tests${test ? `/${test.id}` : ''}`, { method: test ? 'PUT' : 'POST', body: JSON.stringify(payload) }); closeModal(); toast('✅ Test saqlandi'); showTests(); } catch (error) { showFormError('#test-error', error); } });
}
...
function openModal(content: string): void { modalRoot.innerHTML = `<div class="modal-backdrop"><div class="modal">${content}</div></div>`; modalRoot.querySelectorAll('[data-close]').forEach(el => el.addEventListener('click', closeModal)); }
function closeModal(): void { modalRoot.innerHTML = ''; }
function showFormError(selector: string, error: unknown): void {
